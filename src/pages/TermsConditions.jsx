// import React from "react";

// function TermsConditions() {
//   return (
//     <div className="max-w-5xl mx-auto p-12 my-20 text-primary shadow-lg shadow-primary rounded-3xl ">
//       <h1 className="text-3xl font-bold mb-6">Terms and Conditions</h1>
//       <p className="italic mb-6">Effective Date: March 2025</p>

//       <p className="mb-4 italic text-center">
//         Welcome to <strong>Longevidade360</strong>. Please read these Terms and
//         Conditions carefully before using our services.
//       </p>
//       <p className="mb-4 italic text-center">
//         By using our website and services, you agree to be bound by these Terms
//         and Conditions.
//       </p>

//       <h2 className="text-2xl font-semibold mt-8 mb-4">Nature of Services</h2>
//       <p className="mb-4">
//         <strong>Longevidade360</strong> provides{" "}
//         <strong>online medical consultations via videocall</strong> for
//         non-emergency health issues. Our services include, but are not limited
//         to:
//       </p>
//       <ul className="list-disc pl-6 mb-4 space-y-1">
//         <li>
//           Management of any general acute or chronic health issues like fever,
//           cough, cold, flu, gastrointestinal issues, skin conditions, etc.
//           excluding emergencies.
//         </li>
//         <li>Guidance on healthy lifestyle and habit formation</li>
//         <li>
//           Management of pre-diagnosed metabolic diseases like, Hypertension,
//           Diabetes, Hiperlipidemia, thyroid gland malfunction, etc.
//         </li>
//         <li>
//           Medication maintenance (for patients with valid prescriptions from
//           other countries or with sign and symptoms of any chronic condition)
//         </li>
//         <li>
//           Screening tests and check-up recommendations according to age and
//           gender
//         </li>
//         <li>Nutrition, diet and activity recommendations for weight issues</li>
//         <li>Supplement advice</li>
//         <li>Vaccination </li>
//         <li>Pre-travel health advice</li>
//         <li>Mental health support (non-urgent)</li>
//       </ul>
//       <p className="mb-4">
//         Our minimum consultation time is <strong>15 minutes</strong>, though
//         consultations may take longer as needed at the doctor's discretion.
//       </p>

//       <h2 className="text-2xl font-semibold mt-8 mb-4">
//         Language of Consultation
//       </h2>
//       <p className="mb-4">
//         Our services are designed for all the people who speak portuguese;
//         moreover, can be ideal for those{" "}
//         <strong>who cannot speak Portuguese</strong> but can communicate
//         effectively in <strong>English or Persian</strong>. Please ensure you
//         are comfortable communicating in one of these 3 languages before booking
//         a consultation.
//       </p>

//       <h2 className="text-2xl font-semibold mt-8 mb-4">Prescription Policy</h2>
//       <p className="mb-4">
//         We may <strong>prescribe</strong> and support{" "}
//         <strong>continuation of medication</strong>, request{" "}
//         <strong>lab tests</strong>, <strong>imaging </strong>
//         and other <strong>paraclinical evaluations</strong>, according to SNS
//         regulations in Portugal, for patients who:
//       </p>
//       <ul className="list-disc pl-6 mb-4 space-y-1">
//         <li>
//           Have any acute or chronic, non-emergency, general medicine health
//           issues that require evaluation, treatment or medication. People with
//           portuguese health number (Nº de utente) will have the prescription
//           confirmation on their SNS application (if the medication exists and
//           supported by SNS).
//         </li>
//         <li>
//           Have <strong>valid prescriptions</strong> from their doctors in their
//           home countries (even if not issued by a portuguese doctor).
//         </li>
//         <li>
//           Are still experiencing signs and symptoms requiring ongoing treatment.
//         </li>
//       </ul>
//       <p className="mb-4 italic text-center text-primary">
//         However, prescription renewal remains at the sole discretion of the
//         consulting doctor after assessment. <strong>Longevidade360</strong>{" "}
//         reserves the right to decline prescription renewals if it is clinically
//         inappropriate.
//       </p>

//       <h2 className="text-2xl font-semibold mt-8 mb-4">
//         Not for Emergency Use
//       </h2>
//       <p className="mb-4">
//         <strong>IMPORTANT:</strong> <strong>Longevidade360</strong> is{" "}
//         <strong>not intended for emergencies</strong> or urgent care.
//       </p>
//       <ul className="list-disc pl-6 mb-4 space-y-1">
//         <li>
//           In case of medical emergencies, call 112 immediately in Portugal.
//         </li>
//         <li>
//           For medical leave certificates or official sick notes, please contact
//           your family doctor (MGF: Medicina Geral e Familiar) or{" "}
//           <strong>Saúde 24</strong> at 808 24 24 24.
//         </li>
//       </ul>
//       <p className="mb-4">
//         Our service <strong>cannot replace the role of family medicine</strong>{" "}
//         or your primary healthcare provider. We encourage all users to maintain
//         regular follow-up with their local healthcare system.
//       </p>

//       <h2 className="text-2xl font-semibold mt-8 mb-4">Booking and Payment</h2>
//       <p className="mb-4">
//         - Consultations are carried out <strong>via videocall</strong>.<br />-
//         After submitting a request through our website, you will receive an{" "}
//         <strong>Email or WhatsApp message</strong> to arrange the date and time
//         of the consultation, as well as payment instructions.
//         <br />- The consultation fee is <strong>fixed</strong> and{" "}
//         <strong>not covered by insurance</strong>.<br />- Payment must be
//         completed before the scheduled consultation.
//       </p>
//       <h2 className="text-2xl font-semibold mt-8 mb-4">
//         User Responsibilities
//       </h2>
//       <p className="mb-4">
//         By using <strong>Longevidade360</strong> services, you agree to:
//       </p>
//       <ul className="list-disc pl-6 mb-4 space-y-1">
//         <li>
//           Provide accurate, complete, and honest information about your medical
//           history, symptoms, and prescriptions.
//         </li>
//         <li>
//           Provide your ID number (Nº de utente) if you have one, to facilitate
//           prescription confirmation through the SNS application.
//         </li>
//         <li>
//           Provide a valid email address and/or WhatsApp number for communication
//           and appointment scheduling.
//         </li>
//         <li>
//           Provide a valid ID for identification purposes, if requested by the
//           consulting doctor.
//         </li>
//         <li>
//           Attend your scheduled videocall on time in a quiet place with proper
//           internet connection.
//         </li>
//         <li>
//           Understand that the consulting doctor may decline treatment or
//           referrals if deemed clinically inappropriate.
//         </li>
//         <li>
//           Use the service responsibly, understanding its{" "}
//           <strong>limitations</strong> and <strong>non-emergency</strong>{" "}
//           nature.
//         </li>
//       </ul>

//       <h2 className="text-2xl font-semibold mt-8 mb-4">
//         Limitations of Service
//       </h2>
//       <p className="mb-4">
//         - Our service is within scope of general medicine in Portugal and not
//         specialty consultation substitute. examinations.
//         <br />
//         - We do not provide in-person physical examinations, the consultation is
//         carried out through videocall.
//         <br />
//         - We do not issue medical leave certificates or official sick notes.
//         <br />
//         - We do not manage emergency conditions, life-threatening symptoms, or
//         severe mental health crises.
//         <br />- Our consultations are advisory and{" "}
//         <strong>
//           do not replace the role of your family doctor or the national health
//           service
//         </strong>
//         .
//       </p>

//       <h2 className="text-2xl font-semibold mt-8 mb-4">
//         Privacy and Confidentiality
//       </h2>
//       <p className="mb-4">
//         Your personal and medical information will be treated with strict
//         confidentiality and handled in accordance with our{" "}
//         <a href="/privacy-policy" className="text-primary underline">
//           Privacy Policy
//         </a>
//         . We use secure communication channels to arrange consultations and
//         process payments.
//       </p>
//       <p>We do not share your personal information with third parties.</p>

//       <h2 className="text-2xl font-semibold mt-8 mb-4">
//         Changes to These Terms
//       </h2>
//       <p className="mb-4">
//         <strong>Longevidade360</strong> reserves the right to update these Terms
//         and Conditions at any time. Any changes will be posted on this page with
//         an updated effective date.
//       </p>

//       <h2 className="text-2xl font-semibold mt-8 mb-4">Contact</h2>
//       <p className="mb-4">
//         For any questions or concerns about these Terms and Conditions, please
//         contact us at:
//       </p>
//       <ul className="list-disc pl-6 mb-4 space-y-1">
//         <li>Email: longevidade360.pt@gmail.com</li>
//         <li>
//           Website:{" "}
//           <a href="/contact-us" className="text-primary underline">
//             longevidade360.pt/contact-us
//           </a>
//         </li>
//       </ul>
//     </div>
//   );
// }

// export default TermsConditions;

import React from "react";
import { useTranslation } from "react-i18next";

function TermsConditions() {
  const { t } = useTranslation();

  return (
    <div className="max-w-5xl mx-auto p-12 my-20 text-primary shadow-lg shadow-primary rounded-3xl">
      <h1 className="text-3xl font-bold mb-6">{t("terms.title")}</h1>
      <p className="italic mb-6">{t("terms.effectiveDate")}</p>

      <p
        className="mb-4 italic text-center"
        dangerouslySetInnerHTML={{ __html: t("terms.intro1") }}
      />
      <p className="mb-4 italic text-center">{t("terms.intro2")}</p>

      <h2 className="text-2xl font-semibold mt-8 mb-4">
        {t("terms.natureTitle")}
      </h2>
      <p
        className="mb-4"
        dangerouslySetInnerHTML={{ __html: t("terms.natureText") }}
      />
      <ul className="list-disc pl-6 mb-4 space-y-1">
        <li>{t("terms.natureList1")}</li>
        <li>{t("terms.natureList2")}</li>
        <li>{t("terms.natureList3")}</li>
        <li>{t("terms.natureList4")}</li>
        <li>{t("terms.natureList5")}</li>
        <li>{t("terms.natureList6")}</li>
        <li>{t("terms.natureList7")}</li>
        <li>{t("terms.natureList8")}</li>
        <li>{t("terms.natureList9")}</li>
        <li>{t("terms.natureList10")}</li>
      </ul>
      <p
        className="mb-4"
        dangerouslySetInnerHTML={{ __html: t("terms.natureNote") }}
      />

      <h2 className="text-2xl font-semibold mt-8 mb-4">
        {t("terms.languageTitle")}
      </h2>
      <p
        className="mb-4"
        dangerouslySetInnerHTML={{ __html: t("terms.languageText") }}
      />

      <h2 className="text-2xl font-semibold mt-8 mb-4">
        {t("terms.prescriptionTitle")}
      </h2>
      <p
        className="mb-4"
        dangerouslySetInnerHTML={{ __html: t("terms.prescriptionText") }}
      />
      <ul className="list-disc pl-6 mb-4 space-y-1">
        <li>{t("terms.prescriptionList1")}</li>
        <li>{t("terms.prescriptionList2")}</li>
        <li>{t("terms.prescriptionList3")}</li>
      </ul>
      <p
        className="mb-4 italic text-center text-primary"
        dangerouslySetInnerHTML={{ __html: t("terms.prescriptionDisclaimer") }}
      />

      <h2 className="text-2xl font-semibold mt-8 mb-4">
        {t("terms.emergencyTitle")}
      </h2>
      <p
        className="mb-4"
        dangerouslySetInnerHTML={{ __html: t("terms.emergencyText") }}
      />
      <ul className="list-disc pl-6 mb-4 space-y-1">
        <li>{t("terms.emergencyList1")}</li>
        <li>{t("terms.emergencyList2")}</li>
      </ul>
      <p className="mb-4">{t("terms.emergencyNote")}</p>

      <h2 className="text-2xl font-semibold mt-8 mb-4">
        {t("terms.bookingTitle")}
      </h2>
      <p
        className="mb-4"
        dangerouslySetInnerHTML={{ __html: t("terms.bookingText") }}
      />

      <h2 className="text-2xl font-semibold mt-8 mb-4">
        {t("terms.userTitle")}
      </h2>
      <p
        className="mb-4"
        dangerouslySetInnerHTML={{ __html: t("terms.userText") }}
      />
      <ul className="list-disc pl-6 mb-4 space-y-1">
        <li>{t("terms.userList1")}</li>
        <li>{t("terms.userList2")}</li>
        <li>{t("terms.userList3")}</li>
        <li>{t("terms.userList4")}</li>
        <li>{t("terms.userList5")}</li>
        <li>{t("terms.userList6")}</li>
        <li>{t("terms.userList7")}</li>
      </ul>

      <h2 className="text-2xl font-semibold mt-8 mb-4">
        {t("terms.limitsTitle")}
      </h2>
      <p
        className="mb-4"
        dangerouslySetInnerHTML={{ __html: t("terms.limitsText") }}
      />

      <h2 className="text-2xl font-semibold mt-8 mb-4">
        {t("terms.privacyTitle")}
      </h2>
      <p
        className="mb-4"
        dangerouslySetInnerHTML={{ __html: t("terms.privacyText1") }}
      />
      <p>{t("terms.privacyText2")}</p>

      <h2 className="text-2xl font-semibold mt-8 mb-4">
        {t("terms.changesTitle")}
      </h2>
      <p
        className="mb-4"
        dangerouslySetInnerHTML={{ __html: t("terms.changesText") }}
      />

      <h2 className="text-2xl font-semibold mt-8 mb-4">
        {t("terms.contactTitle")}
      </h2>
      <p className="mb-4">{t("terms.contactText")}</p>
      <ul className="list-disc pl-6 mb-4 space-y-1">
        <li>{t("terms.contactEmail")}</li>
        <li dangerouslySetInnerHTML={{ __html: t("terms.contactWebsite") }} />
      </ul>
    </div>
  );
}

export default TermsConditions;
